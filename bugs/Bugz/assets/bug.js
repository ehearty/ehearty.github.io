...
var Bug = {
    ...
    initialize: function(transform, options) {
        ...
        this.zoom = this.random(this.options.zoom, 10) / 8;
    ...
    die: function() {
        killcount++;
        if (killcount > 999){killcount=0;}
        $('#killcount').html(killcount);
        this.stop();
        //pick death style:
        var deathType = this.random(0, this.options.numDeathTypes - 1);

        this.alive = false;
        //this.dieStationary(deathType);
        this.drop(deathType);
        var me = this;
        $(this.bug).animate({opacity:"0"},2000,function(){me.reset();me.options.canFly ? me.flyIn() : me.walkIn();});
    },
    
    dieStationary: function(deathType){
